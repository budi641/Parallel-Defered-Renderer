...
glm::vec3 materialF0 = glm::vec3(0.04f);
...
int main(int /*argc*/, char* /*argv*/[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Parallel-Deferred-Renderer", glfwMonitor, NULL);
    ...
    // Setup light meshes
    lightPoint1.lightMesh.setShape("cube", lightPointPosition1);
    lightPoint1.lightMesh.setShapeScale(glm::vec3(0.1f));
    
    lightPoint2.lightMesh.setShape("cube", lightPointPosition2);
    lightPoint2.lightMesh.setShapeScale(glm::vec3(0.1f));
    
    lightPoint3.lightMesh.setShape("cube", lightPointPosition3);
    lightPoint3.lightMesh.setShapeScale(glm::vec3(0.1f));
...
void renderGeometryPass() {
    ...
    // ✅ CRITICAL: Set viewport before EVERY rendering pass
    glViewport(0, 0, WIDTH, HEIGHT);
...
void renderLightingPass() {
    ...
    // Bind the postprocess FBO
    glBindFramebuffer(GL_FRAMEBUFFER, postprocessFBO);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // Prepare camera matrices (CPU work)
    glm::mat4 view = camera.GetViewMatrix();
    glm::mat4 projection = glm::perspective(camera.cameraFOV, (float)WIDTH / (float)HEIGHT, 0.1f, 100.0f);
...
void renderForwardPass() 
{
    ...
    double startLightPrep = omp_get_wtime();
    double endLightPrep = omp_get_wtime();
    double startDraw = omp_get_wtime();
    double endDraw = omp_get_wtime();
    ...
    if (pointMode)
    {
        ...
        // ✅ FIXED: Simple parallel for (no nesting)
        #pragma omp parallel for schedule(static) if(Light::lightPointList.size() > 2)
...
void imGuiSetup()
{
    ...
    ImGui::Begin("Parallel-Deferred-Renderer", &guiIsOpen, ImVec2(0, 0), 0.5f, ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_NoSavedSettings);
